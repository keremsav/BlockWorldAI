...
void Generate_HashTable_Key(const State *const state, unsigned char* key) 
{
    ...
    for (i = 0; i < position->len_p; i++) {
        key[i] = 'P'+ i + position->position_p[i]  ; // This is hash like P1A
    }

    //key[i++] = '_';

    for (int j = 0; j < position->len_q; j++) {
        i++;
        key[i] = 'Q' + i + position->position_q[j] ; // Hashed like Q2E

    }

    //key[i++] = '_';

    for (int y = 0; y < position->len_r; y++) {
        i++;
        key[i] = 'R' + i +position->position_r[y]; //Hashed like R3C
    }
