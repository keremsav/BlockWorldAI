...
{   Queue *temp_queue;
    ...
    if(Empty(*frontier)) {
        ...
        *frontier = new_queue;
        ...
        double child_cost = child->path_cost + child->state.h_n;
        double frontier_cost = (*frontier)->node->path_cost + (*frontier)->node->state.h_n;

        if (child_cost < frontier_cost) { // Child has lower cost
            new_queue->next = *frontier;
            *frontier = new_queue;
        }
        else {
            for (temp_queue = *frontier; temp_queue->next != NULL; temp_queue = temp_queue->next) {
                double next_cost = temp_queue->next->node->path_cost + temp_queue->next->node->state.h_n;

                if (child_cost < next_cost) {
                    new_queue->next = temp_queue->next;
                    temp_queue->next = new_queue;
                    return;
                }
            }
            // If child has highest cost, insert at the end
            temp_queue->next = new_queue;
            new_queue->next = NULL;
        }
    }
